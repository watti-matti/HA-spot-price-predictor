...
## 8. Suggested release notes (for the GitHub commit)

> **v2.2.0 — D(k) cheap/peak split + DtACI online calibration**
>
> **Hourly model retrained** with Fingrid nuclear features: MAE 24.70 → 23.94 EUR/MWh (−3.1 %), R² 0.386 → 0.515 (+33 %).
>
> **Duration model** now exposes both `dk_cheap_eur_kwh[12]` (cheapest k hours, k=1..12, non-decreasing) and `dk_peak_eur_kwh[12]` (priciest k hours, k=1..12, non-increasing) — twice the decision-relevant range vs the legacy 24-element cumulative D(k). Legacy attribute kept for one transition release.
>
> **Optional DtACI online calibration** — enable `enable_dtaci_dk` in the integration options to wrap the duration forecast with adaptive 90 % prediction intervals (Gibbs & Candès, JMLR 2024) and per-(direction, k) bias EMA. Validation on 4 zones × 700–914 days shows 4–13 % MAE reduction and ±5 pp coverage closure on under-covering zones.
>
> **New Lovelace card** at `docs/yaml_examples/dtaci_diagnostics_card.yaml` — KPI tiles, per-k coverage bars, bias EMA chart, threshold reference. Driven entirely by the new sensor diagnostic attributes.
>
> Backward compatible: existing automations and cards keep working.
