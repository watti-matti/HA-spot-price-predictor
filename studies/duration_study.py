"""Duration-Sorted Price Distribution — Full-History + Forgetting Factor Study

Predicts D(k) = average consumer price for the k cheapest hours in a day.
Uses Ridge regression per (segment, duration_level) + PAVA isotonic post-processing.

Key changes from v1:
  - Expanding-window evaluation across full 4-year history
  - Forgetting factor λ sweep to find optimal exponential decay
  - Spearman rank correlation as primary optimization metric
  - Contour plot with integer c/kWh isolines (replaces pixel heatmap)
"""
...
for seg_name in SEGMENTS:
    ...
    for k in range(n_dur):
        Y[k] = np.array([math.log(d["duration_curve"][k] + LOG_OFFSET) for d in data])
...
<title>Duration-Sorted Price Distribution</title>
...
<body>
<h1>Duration-Sorted Price Distribution</h1>
<p class="sub">D(k) = avg consumer price for cheapest k hours.
Ridge + PAVA | expanding window | forgetting factor optimized.</p>
...
<li><strong>Half-life = 69 days</strong> &mdash; the model structurally cannot learn seasonal patterns
with period &gt; ~4 months; annual winter&ndash;summer seasonality is progressively forgotten.</li>
<li><strong>No regime detection</strong> &mdash; &lambda; decays smoothly regardless of market regime shifts.
A sudden shock takes ~69 days to fully adapt.</li>
...
<tr><td>No regime detection</td>
    <td>~69 days to adapt after market shocks</td>
...
<footer>Duration-Sorted Price Distribution | Ridge + PAVA |
...
(function(){
  ...
  const step=Math.max(1,Math.floor(rl.length/30));
  const labels=rl.map((r,i)=>i%step===0?r.date.substring(0,7):'');
  new Chart(document.getElementById('rollChart').getContext('2d'),{type:'line',
    ...
    options:{responsive:true,animation:false,
      ...
      scales:{x:{ticks:{color:'#e2e8f0',font:{size:9},maxRotation:0,autoSkip:true,maxTicksLimit:15},grid:{color:'#1e293b'}},
...
with open("output/duration_study.html", "w", encoding="utf-8") as f:
    f.write(html)
print("\nDashboard saved to: output/duration_study.html")
